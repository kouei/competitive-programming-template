...
/// <exception cref="invalid_argument">Thrown when the size of str, sa and LCP
...
void createSuffixArray(const string &str, vector<int> &sa, vector<int> &LCP) {
  ...
  vector<int> s(
      N +
      3); // 可以理解为往原字符串末尾填充了3个'\0'字符，填充什么字符不重要，只要保证填充的字符是当前字典里最小的字符就行
...
/// array.</param> <param name="n">Length of s.</param> <param name="K">Maximum
/// character in dictionary. The dictionary is represented as {1..K}.</param>
void makeSuffixArray(const vector<int> &s, vector<int> &SA, int n, int K) {
  int n0 =
      (n + 2) /
      3; // s0的长度，可以理解为((n - 0) + 2) /
         // 3，因为s0在开头没有跳过任何字符，这里的+2是为了实现除以3之后向上取整
  int n1 =
      (n + 1) /
      3; // s1的长度，可以理解为((n - 1) + 2) /
         // 3，因为s1在开头跳过了1个字符，这里的+2是为了实现除以3之后向上取整
  int n2 =
      n /
      3; // s2的长度，可以理解为((n - 2) + 2) /
         // 3，因为s2在开头跳过了2个字符，这里的+2是为了实现除以3之后向上取整
  int t = n0 - n1; // 1 iff n%3 == 1
  int n12 =
      n1 + n2 + t; // 相当于int n12 = n0 + n2，为什么是这样？见94行开始的注释

  vector<int> s12(
      n12 +
      3); // 由于s12这个三元字符串后续可能会需要递归求解后缀数组，所以它也要像s一样，在末尾填充3个空字符（由于是空的三元字符，所以这里是填充3个"\0\0\0"三元空字符）
  ...
  // 这里的i < n + t造成的实际效果是：如果t == 1（即n % 3 ==
  // 1），那么s12里会多插一个三元字符（这个字符会分配给s1），其值为"\0\0\0"（即3个填充字符）
  // 为什么要这么做？这是因为当n % 3 ==
  // 1时，最后一个字符会属于一个s0里的三元字符，而这个三元字符的后面没有对应的来自于s1的三元字符
  ...
  for (int i = 0, j = 0; i < n + t; ++i)
    if (i % 3 != 0) // 如果i % 3 ==
                    // 1，那么此时三元字符(s[i],s[i+1],s[i+2])属于s1；同理如果i
                    // % 3 == 2，那么便属于s2
      s12[j++] =
          i; // 注意，这里的i代表的不是s[i]，而是(s[i],s[i+1],s[i+2])这个三元字符的起始位置
...
int assignNames(const vector<int> &s, vector<int> &s12, vector<int> &SA12,
  ...
  // 经过了radix
  // sort以后，现在SA12里包含了已经排序好的三元字符所对应的下标（以三元字符中的第一个字符在原始数组中的下标来表示）
  ...
  for (int i = 0; i < n12; ++i) {
    ...
    // 这里的SA12[i] /
    // 3是把三元字符的起始字符在原字符串中的下标换算成三元字符串中的下标
    if (SA12[i] % 3 == 1)
...
void computeS0(const vector<int> &s, vector<int> &s0, vector<int> &SA0,
  ...
  for (int i = 0, j = 0; i < n12; ++i)
    // 通过SA12[i] <
    // n0我们跳过了所有S2中的后缀，因为我们只打算根据S1后缀找到对应的S0后缀。
  ...
  radixPass(s0, SA0, s, n0,
            K); // 这里相当于radixPass(s0, SA0, s, 0, n0, K); （即传入offset=0）
}
...
void makeLCPArray(vector<int> &s, const vector<int> &sa, vector<int> &LCP) {
  ...
  for (int i = 0; i < N; ++i)
    rank[sa[i]] =
        i; // rank[sa[i]]表示，从sa[i]这个位置开始的后缀，在所有后缀中的排名
  ...
  for (int i = 0; i < N; ++i)
    if (rank[i] > 0) {
      ...
      // 下面的while循环是计算排名为rank[i]的后缀和排名为rank[i] -
      // 1的后缀的公共前缀长度
      while (s[i + h] == s[j + h])
        ++h;
