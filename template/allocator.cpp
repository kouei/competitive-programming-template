...
template <class T> struct Mallocator {
  ...
  template <class U> bool operator==(const Mallocator<U> &rhs) const noexcept {
    return true;
  }
  template <class U> bool operator!=(const Mallocator<U> &rhs) const noexcept {
    return !(*this == rhs);
...
template <> int *Mallocator<int>::allocate(const size_t n) const {
  if (n == 0) {
    return nullptr;
  } else {
    int *res = &buf[top];
...
template <typename T> using MyVector = vector<T, Mallocator<T>>;
