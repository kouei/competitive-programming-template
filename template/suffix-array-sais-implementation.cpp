...
bool lmsSubstringsAreEqual(const vector<int> &str, const vector<int> &typemap,
...
vector<int> makeSuffixArrayByInducedSorting(const vector<int> &str,
                                            int alphabetSize);
vector<int> guessLMSSort(const vector<int> &str, const vector<int> &bucketSizes,
                         const vector<int> &typemap);
void induceSortL(const vector<int> &str, vector<int> &guessedSuffixArray,
                 const vector<int> &bucketSizes, const vector<int> &typemap);
void induceSortS(const vector<int> &str, vector<int> &guessedSuffixArray,
                 const vector<int> &bucketSizes, const vector<int> &typemap);
tuple<vector<int>, int, vector<int>>
summariseSuffixArray(const vector<int> &str,
                     const vector<int> &guessedSuffixArray,
                     const vector<int> &typemap);
vector<int> makeSummarySuffixArray(const vector<int> &summaryString,
                                   int summaryAlphabetSize);
vector<int> accurateLMSSort(const vector<int> str,
                            const vector<int> &bucketSizes,
                            const vector<int> &typemap,
                            const vector<int> &summarySuffixArray,
                            const vector<int> &summarySuffixOffsets);
...
bool isLMSChar(int offset, const vector<int> &typemap) {
  ...
  return offset > 0 && typemap[offset] == S_TYPE &&
...
vector<int> makeSuffixArrayByInducedSorting(const vector<int> &str,
  ...
  vector<int> summaryString = std::move(get<0>(temp));
  int summaryAlphabetSize = std::move(get<1>(temp));
  vector<int> summarySuffixOffsets = std::move(get<2>(temp));

  // Make a sorted suffix array of the summary string.
  vector<int> summarySuffixArray =
      makeSummarySuffixArray(summaryString, summaryAlphabetSize);

  // Using the suffix array of the summary string, determine exactly
  // where the LMS suffixes should go in our final array.
  vector<int> result = accurateLMSSort(
      str, bucketSizes, typemap, summarySuffixArray, summarySuffixOffsets);
...
summariseSuffixArray(const vector<int> &str,
  ...
  for (int i = 1; i < guessedSuffixArray.size(); ++i) {
    ...
    if (!lmsSubstringsAreEqual(str, typemap, lastLMSSuffixOffset,
                               suffixOffset)) {
