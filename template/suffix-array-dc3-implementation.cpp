...
void computeS0(const vector<int> &s, vector<int> &s0, vector<int> &SA0,
...
/// <param name="LCP">LCP[i] means the length of the longest common prefix
/// between suffix that starts at sa[i] and suffix that starts at sa[i -
/// 1].</param> <exception cref="invalid_argument">Thrown when the size of str,
/// sa and LCP are not the same.</exception>
void createSuffixArray(const string &str, vector<int> &sa, vector<int> &LCP) {
  ...
  vector<int> s(
      N + 3); // Padding three '\0' characters in the end. Padding character
              // Must be the smallest character in the dictionary.
...
/// </summary>
/// <param name="s">Input string, each character is represented as a number in
/// {1..K}.</param> <param name="SA">Suffix array of s.</param> <param
/// name="n">Length of s.</param> <param name="K">Maximum character in the
/// dictionary of s. The dictionary is represented as {1..K}.</param>
void makeSuffixArray(const vector<int> &s, vector<int> &SA, int n, int K) {
  ...
  // S1 = A(BRA)(CAD)(ABR)(A), Len(S1) = 4, notice that we skip 1 character (A)
  // at the beginning. S2 = AB(RAC)(ADA)(BRA), Len(S2) = 3, notice that we skip
  // 2 characters (AB) at the beginning.
  ...
  // The effect of doing this is the same as adding the smallest tri-character
  // ('\0','\0','\0') to the end of S1. This is convenient when we do the
  // "merge" step.
  int n12 = n1 + n2 + t;
  ...
  for (int i = 0, j = 0; i < n + t; ++i) {
    ...
    if (i % 3 != 0) {
      s12[j++] = i; // Here s12[j++] denotes the start position for
                    // tri-character (s[i],s[i+1],s[i+2]) (which is i).
    }
  }
...
/// </summary>
/// <param name="s">Input string, each character is represented as a number in
/// {1..K}.</param> <param name="s12">Concatenation of S1 & S2, which are made
/// by tri-characters.</param> <param name="SA12">Suffix array of s12.</param>
/// <param name="n0">Length of S0.</param>
/// <param name="n12">Length of S12.</param>
/// <param name="K">Maximum character in the dictionary of s. The dictionary is
/// represented as {1..K}.</param>
int assignNames(const vector<int> &s, vector<int> &s12, vector<int> &SA12,
                int n0, int n12, int K) {
  // Radix sort all tri-characters.
  // Since SA12 has not been used yet,
  // here we treat SA12 as the output buffer in radix sort.
  // Here we also treat s12 and SA12 as input buffer and output buffer
  // alternately.
  ...
  // After radix sort, now SA12 contains sorted starting index of all
  // tri-characters in S12. In the next for loop we will count how many unique
  // tri-characters are there and record the number in "name".
  for (int i = 0; i < n12; ++i) {
    auto cur_tri_character =
        make_tuple(s[SA12[i]], s[SA12[i] + 1], s[SA12[i] + 2]);
...
/// <param name="s">Input string, each character is represented as a number in
/// {1..K}.</param> <param name="offset">Offset adding to index in s.</param>
...
/// </summary>
/// <param name="s12">Concatenation of S1 & S2, which are made by
/// tri-characters.</param> <param name="SA12">Suffix array of s12.</param>
/// <param name="n12">Length of S12.</param>
/// <param name="K12">Maximum character in the dictionary of s12. The dictionary
/// is represented as {1..K12}.</param>
void computeS12(vector<int> &s12, vector<int> &SA12, int n12, int K12) {
  // If the number of new names equals to the number of characters in S12,
  // it means every tri-character in S12 is unique.
  // In this case we don't need recursive call to calculate the suffix array of
  // S12, because the start character of a suffix directly decides its relative
  // position among all suffix.
  if (K12 == n12) {
    for (int i = 0; i < n12; ++i) {
      SA12[s12[i] - 1] = i;
    }
    ...
    // If K12 > n12, it means some characters in the dictionary are not used in
    // S12. This is impossible in our algorithm. Hence we always have K12 <=
    // n12. And since we enter this "else" block, it means K12 != n12. So we
    // have K12 < n12 now. (Which means there are duplicate characters in S12)
    makeSuffixArray(s12, SA12, n12, K12);

    // In the following for loop,
    // we imagine that every tri-character in s12 is unique.
    // Hence, we can set unique names to each tri-character in s12 based on
    // SA12. This will make comparison of suffixes in the merge step easier.
    for (int i = 0; i < n12; ++i) {
      s12[SA12[i]] = i + 1;
    }
  }
...
/// </summary>
/// <param name="s">Input string, each character is represented as a number in
/// {1..K}.</param> <param name="s0">S0, which is made by
/// tri-characters.</param> <param name="SA0">Suffix array of s0.</param> <param
/// name="SA12">Suffix array of s12.</param> <param name="n0">Length of
/// S0.</param> <param name="n12">Length of S12.</param> <param name="K">Maximum
/// character in the dictionary of s. The dictionary is represented as
/// {1..K}.</param>
void computeS0(const vector<int> &s, vector<int> &s0, vector<int> &SA0,
               const vector<int> &SA12, int n0, int n12, int K) {
  for (int i = 0, j = 0; i < n12; ++i) {
    if (SA12[i] <
        n0) { // We only want to use S1 to calculate suffix array of S0.
      // SA12[i] is index into s12.
      // Hence, 3 * SA12[i] is index into s.
      // Hence, s0[j++] is the index (into s) of the starting character of the
      // tri-character in S0.
      s0[j++] = 3 * SA12[i];
    }
  }
...
/// <param name="s">Input string, each character is represented as a number in
/// {1..K}.</param> <param name="s12">Concatenation of S1 & S2, which are made
/// by tri-characters.</param> <param name="SA">Suffix array of s.</param>
...
void merge(const vector<int> &s, const vector<int> &s12, vector<int> &SA,
  ...
  // Remember that in function "makeSuffixArray" we pad the smallest
  // tri-character ('\0','\0','\0') to the end of S1. Hence this tri-character
  // will show up in SA12[0], because the suffix starting at this tri-character
  // will surely be the smallest among all suffix. Hence we start from SA12[1]
  // to skip this tri-character because it is not in our original string s.
  while (t != n12 && p != n0) {
...
bool suffix12IsSmaller(const vector<int> &s, const vector<int> &s12,
  ...
  // If the start index (into s) of a S2 suffix that is right after the above S1
  // suffix is p2, then p2 % 3 == 2. We also have p2 == p1 + 1. Hence, p1 / 3 ==
  // p2 / 3. In function "assignNames", we do the following: s12[p1 / 3] = name;
  // s12[p2 / 3 + n0] = name;
  ...
  if (SA12[t] < n0) { // s1 vs s0; can break tie after 1 character
    ...
    return make_tuple(s[i], s[i + 1], s12[SA12[t] - n0 + 1]) <=
           make_tuple(s[j], s[j + 1], s12[j / 3 + n0]);
  }
}
...
/// </summary>
/// <param name="s">Input string, each character is represented as a number in
/// {1..K}.</param> <param name="sa">An existing array to place the suffix
/// array.</param> <param name="LCP">LCP[i] means the length of the longest
/// common prefix between suffix that starts at sa[i] and suffix that starts at
/// sa[i - 1].</param>
void makeLCPArray(vector<int> &s, const vector<int> &sa, vector<int> &LCP) {
  ...
  for (int i = 0; i < N; ++i) {
    rank[sa[i]] =
        i; // rank[sa[i]] means the rank of the suffix starting at sa[i].
  }
  ...
  for (int i = 0; i < N; ++i) {
    if (rank[i] > 0) {
      int j =
          sa[rank[i] - 1]; // starting index of the suffix that has rank[i] - 1.

      // Compute common prefix length of the suffix which has rank[i] and the
      // suffix which has rank[i] - 1.
      while (s[i + h] == s[j + h]) {
        ++h;
      }
      ...
      LCP[rank[i]] = h;
      if (h > 0) {
        // when we enter the next loop, we are actually skipping 1 character at
        // the beginning (which is already matched). Hence we minus 1 from h.
        --h;
      }
    }
  }
